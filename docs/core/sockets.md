...
# FUNCTIONS
1. destroy_socket(sock_t* sock) - free and delete sock_t* instance

2. sock_t* create_socket(int port, int backlog) - creates listening server socket on port from arguments with backlog from arguments

3. sock_t* accept_socket(sock_t* self); - accept client socket from server socket

4. int receive_socket(sock_t* client, void* buffer, uint16_t size, uint16_t readed) - receive from client to buffer with size and with offset readed bytes

5. int send_socket(sock_t* client, void* buffer, uint16_t size, uint16_t sended);- send to client from, buffer with data size size and offset sended bytes

6. void nonblocking_socket(sock_t* sc) - set NONBLOCKING option to socket. 
